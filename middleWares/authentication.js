// passport-jwt	Робить те саме, що і middleWares/authenticate.js але коду там менше на 5 рядків. Складний для розуміння.
...
const authenticate = async (req, res, next) => {
  ...
  try {
    ...
    if (!user || !user.token || user.token !== token) {
      next(
        // HttpError(401, "This user is no longer in the database or he not logged in")
        HttpError(401)
      );
    }
    ...
    next(); // Якщо ж все добре, то просто йдемо далі:
  } catch {
    next(HttpError(401));
  }
  // catch (error) {
  // 	console.log("authenticate >> error:", error);
  // 	next(HttpError(401, "Token not valid, or it time is up, or it was encrypted with a different pass-key")
  // 	);
  // }
};
